/* Synthetic volcomp LOD tree builder shared by tests: a textured volume (waves
 * + blobs — gradient in every direction, near-lossless at q2) written as
 * real volcomp shards + manifest.json, so cpuvol/regvol/the renderer operate on
...
#include <brick.h> /* volcomp (angle include: volcomp src dir) */
#include <shard.h>
...
static inline int st_write_level(const char *root, uint32_t level, const uint32_t nb[3],
  ...
  snprintf(up, sizeof up, "%s/volcomp", root);
  snprintf(dir, sizeof dir, "%s/volcomp/L%u", root, level);
  if ((mkdir(up, 0755) != 0 && errno != EEXIST) ||
      (mkdir(dir, 0755) != 0 && errno != EEXIST))
    return -1;
  snprintf(path, sizeof path, "%s/0_0_0.vcs", dir);
  volcomp_shard_writer *w = volcomp_shard_create(path, 1024, ST_B, level, 2.0f);
  if (!w) return -1;
  ...
  for (uint32_t bi = 0; bi < 512 && rc == 0; bi++) {
    ...
    if (!content || bx >= nb[0] || by >= nb[1] || bz >= nb[2]) {
      rc = volcomp_shard_put_zero(w, bi);
      continue;
    }
    ...
    volcomp_brick_params p = volcomp_brick_defaults(2.0f);
    size_t n = 0;
    if (volcomp_brick_encode(&p, raw, ST_B, &blob, &n) != 0) rc = -1;
    else {
      rc = volcomp_shard_put(w, bi, blob, n);
      free(blob);
      blob = NULL;
    }
  }
 ...
 * axis; nlev 1 or 2 (level 1 halves the dims, rounding up per volcomp-lod).
...
static inline void st_rm_tree(const char *root, uint32_t nlev) {
  char p[720];
  for (uint32_t l = 0; l < nlev; l++) {
    snprintf(p, sizeof p, "%s/volcomp/L%u/0_0_0.vcs", root, l);
    unlink(p);
    snprintf(p, sizeof p, "%s/volcomp/L%u", root, l);
    rmdir(p);
  }
  snprintf(p, sizeof p, "%s/volcomp", root);
  rmdir(p);
