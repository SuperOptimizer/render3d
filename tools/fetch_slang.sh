...
case "$(uname -s)" in
  Darwin) SLANG_OS="macos" ;;
  Linux) SLANG_OS="linux" ;;
  *) echo "fetch_slang: unsupported OS" >&2; exit 1 ;;
esac
...
if [ -f "$STAMP" ] && [ "$(cat "$STAMP")" = "$SLANG_VERSION-$SLANG_OS-$SLANG_ARCH" ]; then
  echo "slangc $SLANG_VERSION already vendored at $DEST"
  exit 0
fi

NAME="slang-$SLANG_VERSION-$SLANG_OS-$SLANG_ARCH.tar.gz"
...
printf '%s' "$SLANG_VERSION-$SLANG_OS-$SLANG_ARCH" > "$STAMP"
"$DEST/bin/slangc" -v
