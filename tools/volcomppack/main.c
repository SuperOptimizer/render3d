...
static int pack_volume(const uint8_t *vol, uint32_t dim, const char *out, float q,
  ...
  if (atomic_load(&j.fail)) {
    fprintf(stderr, "volcomppack: encode failed\n");
    return -1;
  }

  volcomp_shard_writer *w = volcomp_shard_create(out, dim, BD, 0, q);
  if (!w) {
    fprintf(stderr, "volcomppack: cannot create %s\n", out);
    return -1;
  }
  size_t total = 0;
  for (uint32_t b = 0; b < nb; b++) {
    if (volcomp_shard_put(w, b, blobs[b], ns[b]) != 0) return -1;
    total += ns[b];
    free(blobs[b]);
  }
  if (volcomp_shard_close(w) != 0) return -1;
...
int main(int argc, char **argv) {
  ...
  if (strcmp(argv[1], "band") == 0 && argc >= 7) {
    ...
    printf("volcomppack: decoding 3ddct shard %u/%u/%u...\n", Z, Y, X);
