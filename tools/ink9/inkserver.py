...
Loads a koine_machines hybrid_3d2d checkpoint once and serves predictions
over TCP so the viewer can request them per visible region without paying
model-load latency every time.  Run from villa's ink-detection env:

    uv run python tools/ink9/inkserver.py step-075000.pth [--port 9743]

Protocol (little-endian):
  request : 'INK1' u32 layers, h, w   then layers*h*w u8 (Z,H,W layer-major,
            layer stack centered on the surface — rendseg --layers order)
  response: 'INKR' u32 h, w           then h*w f32 probabilities
A zero-filled region returns zeros without touching the GPU. One request is
served at a time (the model is not reentrant); clients queue on connect.
"""
import argparse
import socket
import struct
...
def main():
    ...
    model_args = argparse.Namespace(
        checkpoint=args.checkpoint, model_type="auto", metadata_json=None,
        amp_dtype="auto", gpu_ids=[], compile_model=False, compile_mode="default",
    )
    cm = configure_model(model_args)
    cm, device, _ = prepare_model_for_inference(args=model_args, configured_model=cm)
    cm.model.eval()
    patch = cm.roi_size
    depth = cm.in_chans
    ...
    print(f"inkserver: {args.checkpoint.name} ready on 127.0.0.1:{args.port} "
          f"(patch {patch} stride {stride} depth {depth} device {device})",
          flush=True)
    ...
    def serve(conn):
        ...
        try:
            while True:
                ...
                idx = center_crop_layer_indices(np.arange(layers),
                                                output_depth=depth)
                stack = np.ascontiguousarray(stack[idx])
                t0 = time.time()
                if stack.any():
                    with gpu_lock:
                        pred = predict(cm.model, device, cm.amp_dtype,
                                       cm.preprocessing, stack, patch, stride,
                                       weight_map, args.batch_size)
                else:
                    pred = np.zeros((h, w), dtype=np.float32)
                print(f"inkserver: {w}x{h} in {time.time() - t0:.2f}s",
                      flush=True)
